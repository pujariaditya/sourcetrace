...
you reproduce the feature blocks they were fit on. What each answers:
  ...
  1. Does the SSL layer band matter, i.e. is the shallow 4-7 band a measured choice or an
     ...
     paper after the codec channel: FPR95 0.7771 (layers 1-4) -> 1.0743 (4-7) -> 2.0571
     (8-11), a 1.28-point spread against the channel's 1.55.
...
Variants, all exactly 2048-d so head capacity is identical across arms:
  ...
  band_4_7_meanstd  [mean|std] over layers 4-7      <- reproduces the shipped front-end
  band_1_4_meanstd  [mean|std] over layers 1-4      <- shallower band
