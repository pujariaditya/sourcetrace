...
| SSL front end | `microsoft/wavlm-large`, **frozen**, layers 4–7, mean\\|std pooled → 2048-d |
...
model never sees. Deleting the channel moves FPR95 from 1.14 % to 2.70 % while
...
reproduces `results/ablation/full.json` bit-for-bit — FPR95 1.1428571428571428, OOD-EER
...
|---|---|---|
| Unknown-attack EER ↓ | **9.33 %** | 16.43 % |
| Known-attack EER ↓ | 10.79 % | — |

Measured by the released code on 2026-08-18; raw numbers in
`results/stopa_measured.json`.

9.33 % sits below every baseline STOPA itself reports — mid-30s for AASIST-family
