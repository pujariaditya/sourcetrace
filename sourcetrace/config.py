...
class FeatureConfig:
    ...
    # WavLM layers 1-4, the shallowest band. Selected on the development split (dev OOD-EER 1.95 for 1-4,
    # 2.11 for 4-7, 3.16 for 8-11) and confirmed on evaluation (FPR95 0.50 / 0.91 / 1.58): see
    # results/layer_band_summary.json. Every file in results/ and both checkpoints use this band.
    wavlm_layers: tuple[int, ...] = (1, 2, 3, 4)
...
class ModelConfig:
    ...
    raw_whiten_input_dim: int = 1024               # WavLM mean-half only
    ...
    # The 33->32 residual projection and its FiLM gate are trained with the rest of the head. Setting this
    # False leaves them at their random initialisation (the earlier design): FPR95 0.78 against 0.50 and
    # OOD-EER 3.36 against 2.45 on MLAAD v5, the "fixed projection" row of the paper's Table 2.
    train_codec_path: bool = True
