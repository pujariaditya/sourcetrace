...
class Method:
    ...
    def _seed_all(seed: int) -> None:
        ...
        is what produced every value in ``results/`` — MLAAD v5 FPR95 = 1.14%
    ...
    def fit(
        ...
        #   2. EXCLUSIONS: the champion NEVER adds head_codec / gate_codec params to
        #      the optimizer.  The codec projection and codec gate therefore stay
        #      FROZEN at their initialization for the whole run (they receive
        #      gradients but are never stepped).  We replicate this exactly by
        #      omitting them from ``params`` — the score branch's ``proj_codec`` /
        #      ``gate_codec`` submodules are trained-but-never-updated. The parameter
        #      SET matches the champion exactly; the weight values do not, for the
        #      reconstruction reason above.
    ...
    def embed(self, x: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            t = torch.tensor(np.asarray(x, dtype=np.float32), device=self._device)
            if t.ndim == 1:
                t = t[None, :]
            base = self.head(t).detach().cpu().numpy()      # [N, 288] score branch

        # transfer branch = block PCA-whitened L4-7 mean-half (Original: 780-788).
