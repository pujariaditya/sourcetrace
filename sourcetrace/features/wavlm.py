...
ALREADY been averaged across layers 4-7. **No per-layer identity survives**:
...
class WavLMFrontend:
    ...
    def __init__(
        ...
        Args:
            ...
            layers: Hidden-state indices to pool (default ``(4, 5, 6, 7)``).
                ...
                FPR95 0.7771 at layers 1-4, 1.0743 at 4-7 and 2.0571 at 8-11 --
                monotone in depth. 4-7 is kept because every file in ``results/`` and
                every number in the paper was measured with it; changing it here would
                make those numbers describe a program that no longer exists. Sec. 3.4
                of the paper reports the sweep.
        """
    ...
    def pool_layers(self, ls: torch.Tensor) -> torch.Tensor:
        ...
        single cross-layer average over layers 4-7 -- there are no L4/L5/L6/L7
